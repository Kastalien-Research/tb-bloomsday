...
This document explains the workflow that we follow when building, from ideation to merge-to-main. It is not exhaustive, but it captures the active engineering workflow well enough to keep branch discipline, documentation, and review consistent.
...
- Deleted (with a short handoff note if anything was learned)
...
Prior to beginning work, the chief orchestrator agent should define the units of work that each sub-agent will own. The number of work units and sub-agents should match. Importantly, the chief orchestrator agent should refrain from doing unnecessary manual work themselves, choosing instead to deploy either sub-agents or an agent team to implement the work instead. This protects the chief orchestrator's context from unnecessary work-time tokens best abstracted away in a subagent's summary.
...
- One focused work unit = one sub-agent = one commit
...
- Be specific. "Context seems outdated" is not useful. "The Context section says 'module X is file-backed' but the current implementation now resolves that state from a different subsystem" is useful.
```
